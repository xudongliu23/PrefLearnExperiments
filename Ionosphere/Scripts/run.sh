...
## MaxLearn, UIUP PLP-trees, Ionosphere Dataset, 1 user, with 25839 examples over 315 outcomes.

usr_dir=$3
number_of_users=1
number_of_iterations=20
root_dir=${usr_dir}/Codes/PrefLearnExperiments/Ionosphere
...
function run {
	for (( i=0; i<$number_of_users; i+=1 )); do
      ...
      mkdir -p ${results_dir}/User${i}
      mkdir -p ${results_dir}/User${i}/Training
      mkdir -p ${results_dir}/User${i}/Testing
    fi
