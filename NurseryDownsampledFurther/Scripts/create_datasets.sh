...
for (( i=0; i<$number_of_users; i+=1 )); do
	...
	for (( j=0; j<$number_of_iterations; j+=1 )); do
		shuf ${strictEx_dir}/User${i}/strict_examples${i}.csv > ${strictEx_dir}/User${i}/tmp.csv
		sed -n "1,$training_sample_size p" ${strictEx_dir}/User${i}/tmp.csv > ${strictEx_dir}/User${i}/Training/train${j}
		next=$(($training_sample_size+1))
		sed -n "$next,$ p" ${strictEx_dir}/User${i}/tmp.csv > ${strictEx_dir}/User${i}/Testing/test${j}
		rm ${strictEx_dir}/User${i}/tmp.csv
	done
done
